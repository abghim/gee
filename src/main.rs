...
use crate::stack::*;
use crate::syntax::{
    _Match, Action, ContextInfo, ContextReference, Rule, get_context, get_syntax_info,
    syntax_id_for_filename, syntax_main_and_prototype,
};
...
fn reparse_dirty(view: &mut View) -> String {
    ...
    let visible_end = (view.offset + text_rows + margin)
        .min(view.bufvec.len())
        .saturating_sub(1);
    ...
    if view.recompute > target_end {
        ...
        let cursor = if view.bufvec[cursor_line].is_empty() {
            ...
            view.cursor_x
                .min(view.bufvec[cursor_line].len().saturating_sub(1))
        };
